...
def get_unique_papers_from_sch(max_total: int, paper_query_file_path, output_dir) -> List[Dict]:
    """
    ...
    LOG.info(f"✅ Organized {len(text_data)} papers with title-based IDs.")
...
def load_paper_data(max_paper: int, paper_query_file_path, output_dir) -> tuple[dict, dict, dict]:
    """
...
def ranking_papers(
    ...
    LOG.info("ontology file updated by data from papers")
    ...
    LOG.info("Step 3: Populate mappings from ontology extraction data")
    #print("Step 3: Populate mappings from ontology extraction data")
    #import pdb
    #pdb.set_trace()

    # Step 4: Initialize OpenAI client (make sure get_next_api_key() is implemented) 
    # should change the code just one time load api keys
