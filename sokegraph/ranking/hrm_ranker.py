...
class HRMRanker(BaseRankingMethod):
    def __init__(self,
        ai_tool: AIAgent,
        papers_path,
        ontology_path,
    ...
    def rank(self):
        ...
        # HRM: Neural (preferred) or Fallback linear
        if self.hrm_use_neural and self._hrm_model is not None and len(pair_counts) > 0:
            ...
            # fallback: simple linear HRM
            LOG.info("HRM fallback: using linear α*static + β*pair_count")
            s_dict = {pid: s for pid, s in static_ranked}
            hrm_ranked = []
            for pid, pc in pair_counts.items():
                score = self.hrm_alpha * s_dict.get(pid, 0.0) + self.hrm_beta * float(pc)
                hrm_ranked.append((pid, float(score)))
        ...
        return hrm_outputs_csv, hrm_outputs_all
    ...
    def find_common_papers(
        ...
        for (layer, cat), hits in category_to_papers.items():
            ...
            for pid, _count in hits.items():
                ...
                # static: +1 per matched candidate term
                static_inc = float(len([k for k in kw_set if k in candidate_terms]))
                static_scores[pid] += static_inc
                #import pdb; pdb.set_trace()
                # dynamic: sum of IDFs for matched candidate terms
                dyn_inc = 0.0
                for kw in kw_set:
                    if kw in idf:
        ...
        # ---- Static survivors (opposites filter) ----
        static_ranked = []
    ...
    def _score_with_neural_hrm(
        ...
        with torch.no_grad():
            ...
            scores = self._hrm_model(x).detach().cpu().numpy().tolist()
    ...
    def _train_neural_hrm(self, labels_csv_path: str, save_path: str):
        """
        Train Neural HRM from a CSV of pairwise labels or pointwise labels.
        Expected format (choose one):
          - Pairwise: pid_pos,pid_neg
          - Pointwise: pid,label  (label in {0,1})

        Saves weights to save_path.
        """
        ...
        opt = torch.optim.Adam(self._hrm_model.parameters(), lr=self.hrm_lr)

        loader = DataLoader(dataset, batch_size=self.hrm_batch_size, shuffle=True, drop_last=False)
        ...
        for epoch in range(self.hrm_train_epochs):
            ...
            for batch in loader:
                if dataset.mode == "pair":
                    ...
                    # Minimal featurization: tokenize both pids
                    x_pos = torch.tensor(
                        [self._tokenize_paper(pid, {}, {}, {}, {}) for pid in pid_pos],
                        dtype=torch.long, device=self._hrm_device
                    )
                    x_neg = torch.tensor(
                        [self._tokenize_paper(pid, {}, {}, {}, {}) for pid in pid_neg],
                        dtype=torch.long, device=self._hrm_device
                    )
                    s_pos = self._hrm_model(x_pos)
                    s_neg = self._hrm_model(x_neg)
                    y = torch.ones_like(s_pos)
                    loss = F.margin_ranking_loss(s_pos, s_neg, y, margin=0.1)
                else:
                    pid, ylab = batch
                    x = torch.tensor(
                        [self._tokenize_paper(p, {}, {}, {}, {}) for p in pid],
                        dtype=torch.long, device=self._hrm_device
                    )
                    s = self._hrm_model(x)
                    y = torch.tensor(ylab, dtype=torch.float32, device=self._hrm_device)
            ...
            LOG.info(f"[HRM] epoch {epoch+1}/{self.hrm_train_epochs} loss={total_loss:.4f}")
...
class _NeuralHRM(nn.Module):
    ...
    Minimal neural HRM:
      - Token embedding
      - GRU encoder
      - ACT-like multi-step halting (very light): do K steps and average
      - Linear scorer
    """
    ...
    def forward(self, x: torch.LongTensor) -> torch.Tensor:
        ...
        B, T = x.shape
        h_agg = 0.0
        # ACT-like loop (fixed steps, average states)
        for _ in range(self.max_steps):
            e = self.emb(x)              # [B, T, D]
            _, h = self.gru(e)           # h: [1, B, D]
            h_agg = h_agg + h.squeeze(0) # [B, D]
        h_avg = h_agg / float(self.max_steps)
        s = self.scorer(h_avg).squeeze(-1)  # [B]
        return s
